...
def train_time_series_amount_model(df, date_col="date", label_col="amount"):
    # Extraire des features temporelles simples
    df = df.withColumn("year", year(col(date_col))) \
           .withColumn("month", month(col(date_col))) \
           .withColumn("day", dayofmonth(col(date_col))) \
           .withColumn("day_of_week", dayofweek(col(date_col)))

    # Remplacer les valeurs nulles dans 'amount' par 0 (ou une autre stratégie)
...
def run_pipeline():
    ...
    try:
        ...
        SparkPipelineInit = SparkPipeline('ModelTrain')
